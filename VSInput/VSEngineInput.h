...
namespace VSEngine2
{
	...
	protected:
		...
		MouseRange      m_Cage;							//鼠标范围
		bool			m_bPressed[3];					//存储鼠标是否按下
		bool			m_bReleased[3];					//判断鼠标是否释放
		MousePoint      m_Delta;							//鼠标偏移量
		int                m_lX;
		int                m_lY;
		int					m_lZ;

		//键盘
		TCHAR  m_Keys[256];					//存储键盘键码是否按下
		TCHAR  m_KeysOld[256];				//上一次键盘键码，用来判断键盘是否释放
